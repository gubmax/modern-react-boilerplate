...
import { PATH_RESOLVED_DEV_INDEX_HTML, HtmlMarks, HtmlEntries } from 'server/common/constants'
...
export class DevelopmentRenderService extends RenderService {
  ...
  async renderMainEntry(req: Request, res: Response): Promise<void> {
    const { url } = req
    const { entryDevPath, modulePath } = CONFIG_ENTRIES[HtmlEntries.MAIN]

    let html = this.readHtmlSync()
    html = this.assetCollector.injectUrls(html, [{ url: modulePath, isEntry: true }])
  ...
  async renderInternalErrorEntry(req: Request, res: Response): Promise<void> {
    const { url } = req
    const { entryDevPath, modulePath } = CONFIG_ENTRIES[HtmlEntries.INTERNAL_ERROR]
